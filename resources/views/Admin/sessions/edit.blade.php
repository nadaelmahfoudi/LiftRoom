...
<body class="bg-gray-700">
    ...
    <div class="container mx-auto mt-8">
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-slate-200 text-2xl font-bold">Update Session</h2>
        ...
        <form action="{{ route('sessions.update', $session->id) }}" method="POST" enctype="multipart/form-data" class="bg-gray-500  shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="block text-slate-200 text-sm font-bold mb-2">Name:</label>
                <input type="text" name="name" id="name" value="{{ $session->name }}" class="bg-slate-400 appearance-none border rounded w-full py-2 px-3 text-gray-800 leading-tight focus:outline-none focus:shadow-outline" placeholder="name">
            </div>

            <div class="mb-4">
                <label for="objectif" class="block text-slate-200 text-sm font-bold mb-2">objectif:</label>
                <textarea name="objectif" id="objectif" rows="4" class="resize-none bg-slate-400 appearance-none border rounded w-full py-2 px-3 text-gray-800 leading-tight focus:outline-none focus:shadow-outline" placeholder="objectif">{{ $session->objectif }}</textarea>
            </div>


            <div class="mb-4">
                <label for="duree" class="block text-slate-200 text-sm font-bold mb-2">Durée:</label>
                <input type="text" name="duree" id="duree" value="{{ $session->duree }}" class="bg-slate-400 appearance-none border rounded w-full py-2 px-3 text-gray-800 leading-tight focus:outline-none focus:shadow-outline" placeholder="Durée">
            </div>
            ...
            <div class="text-center">
                <button type="submit" class= " bg-amber-400 text-white py-2 px-4 rounded hover:bg-amber-600 focus:outline-none focus:shadow-outline">Submit</button>
            </div>
        </form>
    </div>
