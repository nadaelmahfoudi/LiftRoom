...
<body class="bg-gray-700">
    ...
    <div class="container mx-auto mt-8">
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-slate-200 text-2xl font-bold">Add New Programme</h2>
        ...
        <form action="{{ route('programmes.store') }}" method="POST" enctype="multipart/form-data" class="bg-gray-500 shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            ...
            <div id="show_item" class="mt-4">
                <div class="flex flex-wrap mb-3 ">
                    ...
                    <div class="w-full md:w-5/12 mb-3 md:mb-0">
                        <label for="input_quantite" class="block text-sm font-medium text-gray-700">Day</label>
                        <input type="text" name="days[]" id="input_quantite" class="form-input mt-1 block w-full h-12 rounded-md shadow-sm bg-gray-400 " placeholder="day">
                    </div>
            ...
            <label for="input_quantite" class="block text-sm font-medium text-slate-200">Objectifs</label>
                    @foreach ($skills as $skill)
                        <li class="relative flex mb-2">
                            <input type="checkbox" id="cat_{{ $skill->id }}" name="skills[]" value="{{ $skill->id }}"
                                class="hidden peer" >
                            <label for="cat_{{ $skill->id }}"
                                class="flex flex-col justify-between w-full p-3 text-gray-500 bg-gray-400 border-2 border-gray-200 rounded-lg cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 peer-checked:border-blue-600  hover:text-gray-600 dark:peer-checked:text-gray-300 peer-checked:text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
